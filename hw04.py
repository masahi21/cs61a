...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
        return tree(new)
    return tree(label(t), [replace_leaf(t, old, new) for t in branches(t)])
...
def make_fib():
    ...
    def fib():
        ...
        result, current, next = current, next, current + next
        return result
    return fib
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    errors = []
    def withdraw(amount, guess):
        nonlocal balance, errors
        if len(errors) == 3:
